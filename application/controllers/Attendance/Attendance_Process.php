        ...
        if (!empty($dtEmp['EmpData'])) {
            ...
            for ($x = 0; $x < count($dtEmp['EmpData']); $x++) {
                ...
                if ($FromDate <= $ToDate) {
                    ...
                    $ShiftDetails['shift'] = $this->Db_model->getfilteredData("select `ID_Roster`,`ShType`,`Day_Type`,`ShiftDay`,`FDate`,`FTime`,`TDate`,`TTime`,`GracePrd`,`HDSession` from tbl_individual_roster where FDate = '$FromDate' AND EmpNo = '$EmpNo' ");
                    ...
                    if ($shift_type == "DU") {
                        ...
                        if ($InTime == null || $InTime == '' || $InTime == '00:00:00') {
                            $dt_in_Records['dt_Records'] = $this->Db_model->getfilteredData("select min(AttTime) as INTime,Enroll_No,AttDate from tbl_u_attendancedata where Enroll_No = '$EmpNo' and AttDate='" . $FromDate . "' AND AttTime BETWEEN '15:00:00' AND '23:00:00' ");
                            $InDate = $dt_in_Records['dt_Records'][0]->AttDate;
                            $InTime = $dt_in_Records['dt_Records'][0]->INTime;
                        }
                        ...
                        $dt_out_Records['dt_out_Records'] = $this->Db_model->getfilteredData("select min(AttTime) as OutTime,Enroll_No,AttDate from tbl_u_attendancedata where Enroll_No='$EmpNo' and AttDate='$oldDate' AND AttTime BETWEEN '15:00:00' AND '23:00:00'");
                        ...
                        $dt_in_Records['dt_Records'] = $this->Db_model->getfilteredData("select min(AttTime) as INTime,Enroll_No,AttDate from tbl_u_attendancedata where Enroll_No = '$EmpNo' and AttDate='" . $oldDate_night_check_2 . "' AND AttTime BETWEEN '15:00:00' AND '23:00:00' AND Status = '0' ");
                        $InTime_check_night = $dt_in_Records['dt_Records'][0]->INTime;

                        $oldDate_night_check = date('Y-m-d', strtotime($FromDate . ' -2 day'));
                        $dt_out_Records['dt_out_Records'] = $this->Db_model->getfilteredData("select min(AttTime) as OutTime,Enroll_No,AttDate from tbl_u_attendancedata where Enroll_No='$EmpNo' and AttDate='$oldDate_night_check' AND AttTime BETWEEN '15:00:00' AND '23:00:00'");
                        ...
                        $dt_in_Records['dt_Records'] = $this->Db_model->getfilteredData("select min(AttTime) as INTime,Enroll_No,AttDate from tbl_u_attendancedata where Enroll_No = '$EmpNo' and AttDate='" . $FromDate . "' AND AttTime BETWEEN '15:00:00' AND '23:00:00' AND Status = '0' ");
                        $InTime_check_night = $dt_in_Records['dt_Records'][0]->INTime;
                        ...
                        if ($OutTime == null || $OutTime == '' || $OutTime == '00:00:00') {
                            ...
                            if ($OutTime > '04:00:00' && $OutTime < '09:00:00') {
                                // $dt_out_Records['dt_out_Records'] = $this->Db_model->getfilteredData("select max(AttTime) as OutTime,Enroll_No,AttDate from tbl_u_attendancedata where Enroll_No = '$EmpNo' and AttDate='" . $newDate . "' AND AttTime BETWEEN '10:00:00' AND '23:59:59' ");
                                // $OutTime = $dt_out_Records['dt_out_Records'][0]->OutTime;
                                ...
                                if ((empty($dt_in_Records['dt_Records'][1]->INTime))) {
                                    ...
                                    $dt_in_Records['dt_Records'] = $this->Db_model->getfilteredData("select min(AttTime) as INTime,Enroll_No,AttDate from tbl_u_attendancedata where Enroll_No = '$EmpNo' and AttDate='" . $newDate_1 . "' AND AttTime BETWEEN '15:00:00' AND '23:00:00' AND Status = '0' ");
                                    $InTime_check_night = $dt_in_Records['dt_Records'][0]->INTime;

                                    $dt_out_Records['dt_out_Records'] = $this->Db_model->getfilteredData("select min(AttTime) as OutTime,Enroll_No,AttDate from tbl_u_attendancedata where Enroll_No='$EmpNo' and AttDate='$FromDate' AND AttTime BETWEEN '15:00:00' AND '23:00:00'");
                                    $OutTime_check_night = $dt_out_Records['dt_out_Records'][0]->OutTime;
                            ...
                            //next day ekedi hawasa out ekt kalin in ekk thiyenwnn out time eka null krnw
                            // $dt_in_Records['dt_Records'] = $this->Db_model->getfilteredData("select min(AttTime) as INTime,Enroll_No,AttDate from tbl_u_attendancedata where Enroll_No = '$EmpNo' and AttDate='" . $newDate . "' AND AttTime BETWEEN '03:00:00' AND '15:00:00' ");
                            // $InTime_check_nextday = $dt_in_Records['dt_Records'][0]->INTime;
                            // if (!empty($InTime_check_nextday)) {
                            //     $OutDate = '';
                            //     $OutTime = '';
                            // }
                        }
                        ...
                        // //out time ekk thibila eth eya ra shift ekata aye awith
                        // $dt_in_Records['dt_Records'] = $this->Db_model->getfilteredData("select min(AttTime) as INTime,Enroll_No,AttDate from tbl_u_attendancedata where Enroll_No = '$EmpNo' and AttDate='" . $FromDate . "' AND AttTime BETWEEN '12:30:00' and '$OutTime'");
                        // $InTime_check_atnight = $dt_in_Records['dt_Records'][0]->INTime;


                        // if ($OutTime < $InTime_check_atnight) {
                        //     // $newDate = date('Y-m-d', strtotime($FromDate . ' +1 day'));
                        //     $newDate = date('Y-m-d', strtotime($FromDate . ' +1 day'));
                        ...
                        //     $newDate = date('Y-m-d', strtotime($FromDate . ' +1 day'));
                        //     // Get the CheckOut in the nextDay (before 9am)
                        //     $dt_out_Records['dt_out_Records'] = $this->Db_model->getfilteredData("select min(AttTime) as OutTime,Enroll_No,AttDate from tbl_u_attendancedata where Enroll_No='$EmpNo' and AttDate='$newDate' AND AttTime <'11:59:59' and Status = '1' ");
                        //     $OutDate = $dt_out_Records['dt_out_Records'][0]->AttDate;
                        //     $OutTime = $dt_out_Records['dt_out_Records'][0]->OutTime;
                        // }
                        ...
                        if ($OutTime != '' && $InTime != $OutTime &&  $shift_type == 'DU' && $OutTime != "00:00:00") {
                    ...
                    if ($shift_type == "EX") {
                        ...
                        if ($settings[0]->Dot_f_holyday == 1) {
                            ...
                            // Get the CheckIN
                            $dt_in_Records['dt_Records'] = $this->Db_model->getfilteredData("select min(AttTime) as INTime,Enroll_No,AttDate from tbl_u_attendancedata where Enroll_No='$EmpNo' and AttDate='" . $FromDate . "' AND AttTime BETWEEN '06:00:00' AND '15:00:00' ");
                            $InDate = $dt_in_Records['dt_Records'][0]->AttDate;
                            $InTime = $dt_in_Records['dt_Records'][0]->INTime;

                            // Get the CheckOut
                            $dt_out_Records['dt_out_Records'] = $this->Db_model->getfilteredData("select max(AttTime) as OutTime,Enroll_No,AttDate from tbl_u_attendancedata where Enroll_No='$EmpNo' and AttDate='" . $FromDate . "' AND AttTime BETWEEN '10:00:00' AND '23:59:59' ");
                            $OutDate = $dt_out_Records['dt_out_Records'][0]->AttDate;
                            $OutTime = $dt_out_Records['dt_out_Records'][0]->OutTime;
